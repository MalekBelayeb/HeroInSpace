...
public class PlayerController2 : MonoBehaviour {
	...
	void Update () {
		...
		if (!inWater){
			...
			if (characterController && characterController.enabled){
				...
				if (characterController.velocity.y < 0.5 && moveDirection.y > 0.5 ){
					headHit = true;
				}
				else {
					headHit = false;
				...
				//checking to see if player's head hit the ceiling
				if (yPos == transform.position.y && rigidBody.velocity.y + 0.1f > yVel && !jumpPerformed && noCollisionTimer == 0 && !sliding && moveDirection.y > 0 ){
	...
	void FixedUpdate () {
		...
		if (!collidingWithWater && noCollisionWithWaterSplashTimer < 5 &&  (swimming.splashEffect != null && Physics.Linecast(new Vector3(transform.position.x, currentHeadPosition - jumpInHeight2, transform.position.z), transform.position, out hit, collisionLayers))){
	...
	void CheckForCharacterController () {
		...
		if (swimming.allowColliderRotationByUsingRigidbody){
			...
			else if (playerUsesCC && canChangeCC){
				if (!characterController || !characterController.enabled){
	...
	void CheckIfWallJumpIsPossible () {
		...
		&&  noCollisionTimer < 5 && directionVector.magnitude >= wallJumping.inputPercentageNeededToWallJump/100 && hit.transform.tag != climbableTag2){
		...
		if (!headHit && wallJumping.allowWallJumping  && !wallIsClimbable && !turnBack && !back2){
	...
	void PerformWallJump () {
		...
		if (animator != null){
			if (!currentlyOnWall && (animator.GetFloat("wallState") == 3 || animator.GetFloat("wallState") == 4) ){
	...
	void CheckForWallRigidbody () {
		...
		if (characterController && characterController.enabled){
			//width
			colliderLeft = characterController.bounds.center - (characterController.radius*transform.right);
			colliderRight = characterController.bounds.center + (characterController.radius*transform.right);
			//length
			colliderBack = characterController.bounds.center - (characterController.radius*transform.forward);
			colliderFront = characterController.bounds.center + (characterController.radius*transform.forward);
		}
		else if (rigidBody){
			if (GetComponent<CapsuleCollider>()){
				//width
				colliderLeft = GetComponent<Collider>().bounds.center - (GetComponent<CapsuleCollider>().radius*transform.right);
				colliderRight = GetComponent<Collider>().bounds.center + (GetComponent<CapsuleCollider>().radius*transform.right);
				colliderLeftLonger = GetComponent<Collider>().bounds.center - (GetComponent<CapsuleCollider>().radius*1.5f*transform.right);
				colliderRightLonger = GetComponent<Collider>().bounds.center + (GetComponent<CapsuleCollider>().radius*1.5f*transform.right);
				colliderLeftLonger2 = GetComponent<Collider>().bounds.center - (GetComponent<CapsuleCollider>().radius*2*transform.right);
				colliderRightLonger2 = GetComponent<Collider>().bounds.center + (GetComponent<CapsuleCollider>().radius*2*transform.right);
				//length
				colliderBack = GetComponent<Collider>().bounds.center - (GetComponent<CapsuleCollider>().radius*transform.forward);
				colliderFront = GetComponent<Collider>().bounds.center + (GetComponent<CapsuleCollider>().radius*transform.forward);
				colliderBackLonger = GetComponent<Collider>().bounds.center - (GetComponent<CapsuleCollider>().radius*1.5f*transform.forward);
				colliderFrontLonger = GetComponent<Collider>().bounds.center + (GetComponent<CapsuleCollider>().radius*1.5f*transform.forward);
				colliderFrontLonger2 = GetComponent<Collider>().bounds.center + (GetComponent<CapsuleCollider>().radius*2*transform.forward);
				//height
				colliderBottom = GetComponent<Collider>().bounds.center - (GetComponent<CapsuleCollider>().height*0.5f*transform.up);
				colliderTop = GetComponent<Collider>().bounds.center + (GetComponent<CapsuleCollider>().height*0.5f*transform.up);
			}
			else if (GetComponent<SphereCollider>()){
				//width
				colliderLeft = GetComponent<Collider>().bounds.center - (GetComponent<SphereCollider>().radius*transform.right);
				colliderRight = GetComponent<Collider>().bounds.center + (GetComponent<SphereCollider>().radius*transform.right);
				colliderLeftLonger = GetComponent<Collider>().bounds.center - (GetComponent<SphereCollider>().radius*1.5f*transform.right);
				colliderRightLonger = GetComponent<Collider>().bounds.center + (GetComponent<SphereCollider>().radius*1.5f*transform.right);
				colliderLeftLonger2 = GetComponent<Collider>().bounds.center - (GetComponent<SphereCollider>().radius*2*transform.right);
				colliderRightLonger2 = GetComponent<Collider>().bounds.center + (GetComponent<SphereCollider>().radius*2*transform.right);
				//length
				colliderBack = GetComponent<Collider>().bounds.center - (GetComponent<SphereCollider>().radius*transform.forward);
				colliderFront = GetComponent<Collider>().bounds.center + (GetComponent<SphereCollider>().radius*transform.forward);
				colliderBackLonger = GetComponent<Collider>().bounds.center - (GetComponent<SphereCollider>().radius*1.5f*transform.forward);
				colliderFrontLonger = GetComponent<Collider>().bounds.center + (GetComponent<SphereCollider>().radius*1.5f*transform.forward);
				colliderFrontLonger2 = GetComponent<Collider>().bounds.center + (GetComponent<SphereCollider>().radius*2*transform.forward);
				//height
				colliderBottom = GetComponent<Collider>().bounds.center - (GetComponent<SphereCollider>().radius*transform.up);
				colliderTop = GetComponent<Collider>().bounds.center + (GetComponent<SphereCollider>().radius*transform.up);
			}
			else if (GetComponent<BoxCollider>()){
				//width
				colliderLeft = GetComponent<Collider>().bounds.center - (GetComponent<BoxCollider>().size.x*0.5f*transform.right);
				colliderRight = GetComponent<Collider>().bounds.center + (GetComponent<BoxCollider>().size.x*0.5f*transform.right);
				colliderLeftLonger = GetComponent<Collider>().bounds.center - (GetComponent<BoxCollider>().size.x*0.5f*1.5f*transform.right);
				colliderRightLonger = GetComponent<Collider>().bounds.center + (GetComponent<BoxCollider>().size.x*0.5f*1.5f*transform.right);
				colliderLeftLonger2 = GetComponent<Collider>().bounds.center - (GetComponent<BoxCollider>().size.x*0.5f*2*transform.right);
				colliderRightLonger2 = GetComponent<Collider>().bounds.center + (GetComponent<BoxCollider>().size.x*0.5f*2*transform.right);
				//length
				colliderBack = GetComponent<Collider>().bounds.center - (GetComponent<BoxCollider>().size.z*0.5f*transform.forward);
				colliderFront = GetComponent<Collider>().bounds.center + (GetComponent<BoxCollider>().size.z*0.5f*transform.forward);
				colliderBackLonger = GetComponent<Collider>().bounds.center - (GetComponent<BoxCollider>().size.z*0.5f*1.5f*transform.forward);
				colliderFrontLonger = GetComponent<Collider>().bounds.center + (GetComponent<BoxCollider>().size.z*0.5f*1.5f*transform.forward);
				colliderFrontLonger2 = GetComponent<Collider>().bounds.center + (GetComponent<BoxCollider>().size.z*0.5f*2*transform.forward);
				//height
				colliderBottom = GetComponent<Collider>().bounds.center - (GetComponent<BoxCollider>().size.y*0.5f*transform.up);
				colliderTop = GetComponent<Collider>().bounds.center + (GetComponent<BoxCollider>().size.y*0.5f*transform.up);
			}
		}
	...
	void ClimbingWall () {
		...
		if (currentlyClimbingWall && !pullingUp){
			...
			if (Input.GetAxisRaw("Vertical") >= 0 || switchedDirection && !downCanBePressed ){
			...
			if (Input.GetAxisRaw("Vertical") < 0 && Input.GetAxisRaw("Horizontal") == 0 && (!downCanBePressed || reachedBottomPoint && (!dropOffAtBottom2 ||  !dropOffAtFloor2))){
		...
		if (animator != null){
			...
			if (!pullingUp){
				...
				if (currentlyClimbingWall || turnBack || back2){
					...
					if (( animator.GetFloat("climbState") != 0 || animator.GetCurrentAnimatorStateInfo(0).IsName("Climbing")) && !inMidAirFromJump){
						animator.CrossFade("Movement", 0f, -1, 0f);
					}
	...
	void GettingMovementDirection () {
		...
		if ( (noCollisionTimer < 5 || Physics.Raycast(pos, Vector3.down, maxGroundedDistance2, noWaterCollisionLayers))) {
	...
	void LockAxisForSideScrolling () {
		...
		if (noCollisionTimer >= 5 &&  inMidAirFromJump || jumpPerformed || !sliding){
			slidingVector = Vector3.zero;
		}
	...
	void SlopeSliding () {
		...
		if (raycastSlopeAngle > slopeLimit && collisionSlopeAngle < 89 && !jumpPerformed && !inMidAirFromJump && slidePossible && !inBetweenSlidableSurfaces){
			...
			if (noCollisionTimer < 5 ){
				...
				if (noCollisionTimer > 2 &&  moveDirection.y <= -gravity){
		...
		if (!sliding){
			if (movement.slideFriction > 0){
				...
				if (slideMovement != Vector3.zero){
					if (rigidBody && !jumpPerformed && !inMidAirFromJump  && noCollisionTimer < 5 && Physics.Raycast(pos, Vector3.down, out hit, 1f, noWaterCollisionLayers)){
						transform.position = new Vector3(transform.position.x, hit.point.y, transform.position.z);
					}
					moveDirection.x += (slideMovement*(slideSpeed*slopeSlideSpeed2) + new Vector3(0, -8, 0)).x;
					moveDirection.z += (slideMovement*(slideSpeed*slopeSlideSpeed2) + new Vector3(0, -8, 0)).z;
		...
		if (Physics.Raycast(pos, Vector3.down, out hit, 1f, noWaterCollisionLayers) && !rigidBody || Physics.Raycast(pos + transform.forward/10, Vector3.down, out hit, 1f, noWaterCollisionLayers) && rigidBody){
			if ( !jumpPerformed){
				...
				if (characterController && characterController.enabled && (raycastSlopeAngle > 1 || raycastSlopeAngle < -1)){
					...
					//applying a downward force to keep the player from bouncing down slopes
					if ( (noCollisionTimer < 2 || -(moveDirection.y - rigidBody.velocity.y) < transform.position.y && moveDirection.y <= rigidBody.velocity.y + 5 - transform.position.y) && !sliding && !inMidAirFromJump && !inMidAirFromWallJump){
						moveDirection.y -= hit.point.y;
						if (Physics.Raycast(pos + transform.forward/2 + transform.up, Vector3.down, out frontHit, 5f, noWaterCollisionLayers) && Physics.Raycast(pos - transform.forward/2 + transform.up, Vector3.down, out backHit, 5f, noWaterCollisionLayers)){
	...
	void MovePlayer () {
		...
		if (!currentlyOnWall && !currentlyClimbingWall){
			...
			if (characterController && characterController.enabled){
				...
				if ( moveDirection.y >= 0 && (noCollisionTimer > 5 || uphill) && !sliding && !jumpPerformed){
				...
				||  noCollisionTimer < 5)){
					...
					if (( !inMidAirFromJump && !inMidAirFromWallJump || raycastSlopeAngle > slopeLimit && collisionSlopeAngle < 89 && !jumpPerformed && slidePossible || inBetweenSlidableSurfaces)
			...
			//if player is using a Rigidbody
				...
				//applying a downward force to keep the player falling instead of slowly floating to the ground
				if ( Mathf.Abs(rigidBody.velocity.y) > 1f && moveDirection.y >= 0 && (noCollisionTimer > 5 || uphill) && !sliding && !jumpPerformed){
					if (Physics.Raycast(pos, Vector3.down, out hit, 1f, noWaterCollisionLayers) && ((Mathf.Acos(Mathf.Clamp(hit.normal.y, -1f, 1f))) * 57.2958f) <= slopeLimit){
						if ( (noCollisionTimer < 2 || -(moveDirection.y - rigidBody.velocity.y) < transform.position.y && moveDirection.y <= rigidBody.velocity.y + 5 - transform.position.y) && !sliding && !inMidAirFromJump && !inMidAirFromWallJump){
							moveDirection.y -= -transform.position.y + hit.normal.y;
						}
				...
				// move the player if grounded
				//checking for grounded (using the position variable pos)
				if (( !jumpPressed && !jumpPerformed) && !sliding && noCollisionTimer < 5
				&&  Physics.Linecast(pos + maxGroundedHeight2, pos + (maxGroundedDistanceDown * ((raycastSlopeAngle/90) + 1)), out hit, noWaterCollisionLayers)
				&&	Physics.Linecast(pos - transform.forward*(maxGroundedRadius2/2) + maxGroundedHeight2, pos - transform.forward*(maxGroundedRadius2/2) + (maxGroundedDistanceDown * ((raycastSlopeAngle/90) + 1)), out hit, noWaterCollisionLayers)
				&&	Physics.Linecast(pos + transform.forward*(maxGroundedRadius2/2) + maxGroundedHeight2, pos + transform.forward*(maxGroundedRadius2/2) + (maxGroundedDistanceDown * ((raycastSlopeAngle/90) + 1)), out hit, noWaterCollisionLayers)
				&&	Physics.Linecast(pos - transform.right*(maxGroundedRadius2/2) + maxGroundedHeight2, pos - transform.right*(maxGroundedRadius2/2) + (maxGroundedDistanceDown * ((raycastSlopeAngle/90) + 1)), out hit, noWaterCollisionLayers)
				&&	Physics.Linecast(pos + transform.right*(maxGroundedRadius2/2) + maxGroundedHeight2, pos + transform.right*(maxGroundedRadius2/2) + (maxGroundedDistanceDown * ((raycastSlopeAngle/90) + 1)), out hit, noWaterCollisionLayers)){
					...
					//moving player, and avoiding bouncing if on a sloped surface
					if (raycastSlopeAngle <= slopeLimit  && (noCollisionTimer < 2 || -(moveDirection.y - rigidBody.velocity.y) < transform.position.y && moveDirection.y <= rigidBody.velocity.y + 5 - transform.position.y) && !sliding && !inMidAirFromJump && !inMidAirFromWallJump){
	...
	void Swim5 () {
		...
		if (atJumpHeight || reachedTopOfWater && transform.position.y + 0.01f >= inWaterPosY && !Input.GetButton("Jump")){
	...
	void Swim6 () {
		...
		if (characterController && characterController.enabled){
			...
			if (!Physics.Linecast((colliderBottom*9 + colliderTop)/10, (colliderTop*9 + colliderBottom)/10, out hit, collisionLayers)){
				...
				lastRotBeforeCollision = transform.rotation;
			}
				...
				transform.rotation = lastRotBeforeCollision;
			}
	...
	void Swim8 () {
		...
		if ( Vector3.Distance(swimDirection, Vector3.zero) >= 0.01f
	...
	void CheckClimbableEdges () {
		...
		if (!pullingUp){
			...
			else if (!currentlyClimbingWall || currentlyClimbingWall && ( dropOffAtBottom2)){
